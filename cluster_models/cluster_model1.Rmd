...
setwd("~/storage/scratch/users/eo20t062/model1/slim_files") #setting the working directory
...
for(r in 1:reps){
  slim_code_block <- paste0("slim -d repetition=", r, " cluster_model1_nrnr.slim")
...
for(r in 1:reps){
  slim_code_block <- paste0("slim -d repetition=", r, " cluster_model1_nrr.slim")
  #slim_code_block - goes through the amount of repetitions I want
  system(slim_code_block)#call the code block for slim in the terminal via bash, system invokes the OS command specified by command.
}
...
for(r in 1:reps){
  slim_code_block <- paste0("slim -d repetition=", r, " cluster_model1_r.slim")
  #slim_code_block - goes through the amount of repetitions I want
  system(slim_code_block)#call the code block for slim in the terminal via bash, system invokes the OS command specified by command.
}
...
```{r}
setwd("~/storage/scratch/users/eo20t062/model1/slim_output")
...
row_numbers <- seq(100001, 101000, by = 5) #This command will help me name/define the rows. These are the generations from my SLiM output - I had 100 generations and wrote down the heterozygosity for every second generation. 
...
```{r}
setwd("~/storage/scratch/users/eo20t062/model1/slim_output") #setting the working directory
...
matplot( x_values, results_df_nrnr, type = "p", pch = 16, 
        col = adjustcolor("#54BFB7FF", alpha.f = 0.05), # Model nrnr: light blue points, rgb(red, green, blue, alpha) enables me to modify the colors a bit better, alpha is the transparacy
