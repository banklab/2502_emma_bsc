...
This is the R code for my Inbreeding management model 1. I used SLiM to generate data and R to then analyze it. 


## Generating Data in SLiM with Parameters from R 

First I have 3 chunks, which will send information to my slim model - With this callback I can make 20 repetitions of my simulation in SLiM and save the output files. 
...
non-random mating selecting for not related

```{r}
#setwd("~/Documents/GitHub/2502_emma_bsc/Outputs/") #setting the working directory

## all SLiM output files which show nonrandom mating with mate choice for least relatedness are listed
slim_out_files <- list.files(pattern = "_nonrandom_notrelated_")

length(slim_out_files) #tells me how long the list is, I just do this to check if it worked. 
#install.packages("hierfstat")
library("hierfstat") #useful package for analyzing
...
row_numbers <- seq(100001, 100991, by = 20) #define the rownumbers as the generations

results_df_nrnr <- matrix(NA ,nrow = length(slim_out_files), ncol = (length(row_numbers) + 2)) #create a matrix to store all the data, rows for the repetitions and columns for the heterozygosity values across the generations 
colnames(results_df_nrnr) <- c(row_numbers,"reps","seed") #name the columns, first few with the generations and the last two as "reps" and "seed"
length(c(row_numbers,"reps","seed")) #check if it worked
dim(results_df_nrnr) #check if it worked


#now I start with filling in the created matrix - 
for(i in 1:length(slim_out_files)){ 
  ...
  results_df_nrnr[i,"reps"] <- get_the_reps ## fill in the values for repetition in the matrix
  
  results_df_nrnr[i,"seed"] <- get_the_seed ## record the seed for this particular results file and put it in the matrix
  
  # Read the single CSV file, ensure that it recognizes the header
...
# Create a new row to later save the means - NA for "reps" and "seed" columns since they dont have a mean
...
# Extract the mean heterozygosity values from the last row for every column exceot the last two--> Last row, so we can use nrow
mean_values_nrnr <- results_df_nrnr[nrow(results_df_nrnr), 1:(ncol(results_df_nrnr)-2)]

# Create the plot
matplot(
  cycle_numbers,  # X-axis: Generations
  t(heterozygosity_values_nrnr),  # Transpose to align data correctly - this is needed since matplot expects that each column is a separate data series - my separate data series are stored as rows, so i have to transpose them. 
  type = "p",  # "p" for points
  pch = 16,  # Use small dots for points
  col = rgb(0.4, 0.6, 1, 0.2),  # Blue with transparency --> rgb is kind of nice, (red, green, blue, alpha (transparency))
  xlab = "Generation (Cycle)", #name the axis of the plot
  ylab = "Heterozygosity",
  main = "Heterozygosity Across Generations - nonrandom-mating selecting for nonrelatedness", #title of the plot
  cex.main = 0.8  # Makes title smaller
)

# Add the mean line
lines(cycle_numbers, mean_values_nrnr, col = "orange", lwd = 2)  # orange solid line
...
Here i repeat all the steps from before, just for another model in SliM with a different mateChoice callback. For better annotation look above. 
...
Analyzing the output files from SLiM
non-random mating selecting for relatedness

```{r}
#setwd("~/Documents/GitHub/2502_emma_bsc/Outputs/") #setting the working directory
getwd()  # Prints the current working directory
## all SLiM output files which show nonrandom mating with mate choice for highest relatedness
...
row_numbers <- seq(100001, 100991, by = 20) #this will help me create the columns i need in my matrix
...
for(i in 1:length(slim_out_files)){ 
  ...
  # Ensure the column names of this single file are correct
  colnames(df) <- c("cycle", "Heterozygosity")  
...
# Add the mean line
lines(cycle_numbers, mean_values_nrr, col = "orange", lwd = 2)  # orange solid line
...
Now i do exactly the same, for the last model in Slim, which has a mateChoice command selecting mates at random. So the mating will be random...
...
# Assign row name to the last row
...
This chunk combines all the three graphs i just generated into one. This helps me to better compare the outputs...
...
Now I do a plot from only the first 100 generations, to see what exactly is happening at the beginning...
...
# Filter to only the first 100 generations
cutoff <- 100101 
shorting <- cycle_numbers <= cutoff


print(length(cycle_numbers[shorting]))  # Should be > 0
print(dim(t(heterozygosity_only_r)[shorting, ]))  # Should show rows and columns

# Create the plot
matplot(
  cycle_numbers[shorting],  # X-axis: Generations, here the filter is active 
  t(heterozygosity_only_r)[shorting, ],  # Transpose to align data correctly --> I want to cut the columns not the rows, thats why there is this notation with the ,
  type = "p",  # "p" for points
  pch = 16,  # Use small dots for points
  col = rgb(0.4, 0.6, 1, 0.2),  # light Blue with transparency --> rgb is kind of nice, (red, green, blue, alpha)
  xlab = "Generation (Cycle)",
...
# Add the mean line
lines(cycle_numbers[shorting], mean_values_r[shorting], col = "blue", lwd = 2)  # Red solid line

# Now add the other points and lines
# Add non-random selecting for relatedness
matpoints(cycle_numbers[shorting], t(heterozygosity_only_nrr)[shorting, ], pch = 16, col = rgb(1, 0.5, 0.5, 0.2))
lines(cycle_numbers[shorting], mean_values_nrr[shorting], col = "red", lwd = 2)

# add non-rendom selecting for not related
matpoints(cycle_numbers[shorting], t(heterozygosity_only_nrnr)[shorting, ], pch = 16, col = rgb(0.4, 1, 0.4, 0.2))
lines(cycle_numbers[shorting], mean_values_nrnr[shorting], col = "forestgreen", lwd = 2)
